...
struct FWWidgetLarge: View {
    ...
    let fontBold = Font.custom("Avenir-Bold", size: 12)
    ...
    func formatSubtitle(str: String) -> String {
        var subtitle = ""
        let sub = str.split(separator: " ")
        for num in 0..<3 {
            subtitle.append(contentsOf: sub[num])
            subtitle.append(" ")
        }
        let lastSpace = subtitle.lastIndex(of: " ")!
        subtitle.remove(at: lastSpace)
        subtitle.append("...")
